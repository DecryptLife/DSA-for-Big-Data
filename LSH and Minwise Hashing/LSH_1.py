...
for query in queries:
    [hashval,minhash_queries] = minhash(query,2)
...
for ele in query_set:
    [hashval, minhash_queries] = minhash(ele, 2)
    matched = (hashTable.lookup(hashval))
    top_10_list,top_10 = jbrute_heap(ele, matched)
    mean_j_10 += calculateJaccard(ele,[top_10],hashval)
    print()
    print("Top 10 of ",ele," is: ",top_10)

print()
print("Mean Jaccard similarity of top 10: ",mean_j_10/200)
print()
j_start = time.time()
for ele in query_set:
    [hashval,minhash_queries] = minhash((ele,2))
    mean_j_200 += calculateJaccard(ele,matched,hashval)
print("Mean Jaccard: ",mean_j_200/200)
jaccard_query_time = (time.time() - j_start)/200


b_start = time.time()
for ele in query_set:
    mean_brute += brute_jaccard(ele,matched)
print("Mean Brute: ",mean_brute/200)
total_time_brute = time.time() - j_start
print()
print(f'Jaccard Query Time: {jaccard_query_time} seconds')
print(f'Brute Jaccard Time: {(total_time_brute)} seconds')
print(f'Brute Query Jaccard Time: {(total_time_brute)/200} seconds')
